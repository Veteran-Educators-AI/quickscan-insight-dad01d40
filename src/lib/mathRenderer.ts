...
function convertLatex(text: string): string {
  ...
  const sortedLatex = Object.entries(latexCommands).sort(
  ...
  result = result.replace(
    /\\(sin|cos|tan|cot|sec|csc|log|ln|lim|max|min|exp)\b/g,
    "$1",
  );
...
function convertSubscripts(text: string): string {
  ...
  return text.replace(/_{([^}]+)}|_(\d+|[a-z])/gi, (match, braced, simple) => {
    ...
    return content
      .split("")
      .map((char: string) => subscripts[char] || char)
      .join("");
  });
}
...
export function formatCurrency(text: string): string {
  ...
  const hasMoneyContext = moneyContextKeywords.some((keyword) =>
  ...
  result = result.replace(
    /(?<!\$)(?<!\d)\b(\d{1,})\.(00|[0-9]{2})\b(?!\s*(?:dollars?|cents?|%|degrees?|°|cm|m|ft|in|kg|lb|g|oz|ml|L|hours?|minutes?|seconds?|years?|months?|days?|miles?|km))(?!\d)/gi,
    (match, whole, cents) => {
      return `$${whole}.${cents}`;
    },
  );
  ...
  result = result.replace(
    /\b(costs?|priced? at|charges?|pays?|earns?|spends?|saves?|sells? for|bought for|sold for|worth)\s+(?<!\$)(\d+(?:\.\d{2})?)\b(?!\s*(?:dollars?|cents?|%))/gi,
    (match, verb, amount) => {
  ...
  result = result.replace(
    /\b(profit|revenue|income|savings?|balance|total|discount|fee|tax|tip|cost|price|amount|loss)\s+of\s+(?<!\$)(\d+(?:\.\d{2})?)\b(?!\s*(?:dollars?|cents?|%))/gi,
    (match, noun, amount) => {
      const formattedAmount = amount.includes(".") ? amount : `${amount}.00`;
  ...
  result = result.replace(
    /\b(?:is|was|equals?|=|totals?|makes?)\s+(?<!\$)(\d+\.\d{2})(?=\s*[.,;:?!]|\s*$)/gi,
    (match, amount) => {
  ...
  result = result.replace(
    /(\$\d+\.\d{2})(?=\s*[.,;:?!]|\s*$)(?!\s*(?:dollars?|cents?|each|per|for))/gi,
    (match) => `${match} dollars`,
  );
...
export function sanitizeForPDF(text: string): string {
  ...
  // Keep Unicode math symbols that render well in PDFs
  // Only convert truly problematic characters (emojis, corrupted text)
  const safeReplacements: [RegExp, string][] = [
    // Greek letters - keep as readable text for clarity
    [/π/g, "pi"],
    [/θ/g, "theta"],
    [/α/g, "alpha"],
    [/β/g, "beta"],
    ...
    // Square root - KEEP the √ symbol, it renders in most PDF viewers
    // Only add spacing when needed for clarity
    [/(\d)√/g, "$1√"], // Keep coefficient attached: 6√x stays as 6√x

    // Infinity - keep or convert based on readability
    [/∞/g, "infinity"],

    // Comparison operators - KEEP these, they're readable
    [/≤/g, "<="],
    [/≥/g, ">="],
    [/≠/g, "!="],
    [/≈/g, "~"],

    // Basic math operators - keep readable versions
    [/±/g, "+/-"],
    [/×/g, "x"],
    [/÷/g, "/"],
    [/·/g, "*"],
    ...
    // Superscripts - KEEP ² and ³ as they're common and readable
    // Convert extended superscripts to ^n notation
    [/⁺/g, "^+"],
    [/⁻/g, "^-"], // Fix: superscript minus was corrupting
    [/⁼/g, "^="],
    [/⁽/g, "^("],
    [/⁾/g, "^)"],
    [/⁰/g, "^0"],
    [/¹/g, "^1"], // Keep as ^1 for consistency
    [/⁴/g, "^4"],
    [/⁵/g, "^5"],
    [/⁶/g, "^6"],
    [/⁷/g, "^7"],
    ...
    // Fractions in exponents - use simple notation without extra parens
    // These will be inside ^() already, so don't add more parens
    [/½/g, "1/2"],
    [/⅓/g, "1/3"],
    [/⅔/g, "2/3"],
    [/¼/g, "1/4"],
  ...
  // Fix double parentheses that may occur: ^((1/2)) -> ^(1/2)
  result = result.replace(/\^\(\(([^)]+)\)\)/g, "^($1)");

  // Fix consecutive caret notations: ^-^1 -> ^(-1)
