...
import {
  getFormulasForTopics,
  type FormulaCategory,
} from "@/data/formulaReference";
import { ScrapPaperGenerator } from "@/components/print/ScrapPaperGenerator";
import { generateWorksheetPDF, printWorksheetPDF } from "@/lib/pdfGenerator";
...
export function WorksheetBuilder({
  ...
  const [worksheetMode, setWorksheetMode] =
    useState<WorksheetMode>("diagnostic");
  ...
  useEffect(() => {
    if (selectedQuestions.length > 0 && !hasUserEditedTitle) {
      const prefix =
        worksheetMode === "basic_assessment" ? "Assessment: " : "Diagnostic: ";
  ...
  const [includeCoordinateGeometry, setIncludeCoordinateGeometry] =
    useState(false);
  ...
  const [scrapPaperLayout, setScrapPaperLayout] = useState<
    "single" | "split-2" | "split-4"
  >("split-2"); // Scrap paper layout
  ...
  const [editablePrompts, setEditablePrompts] = useState<
    { questionNumber: number; prompt: string }[]
  >([]);

  // Single image regeneration state
  const [regeneratingQuestionNumber, setRegeneratingQuestionNumber] = useState<
    number | null
  >(null);
  ...
  const [lightboxQuestionNumber, setLightboxQuestionNumber] = useState<
    number | null
  >(null);

  // Canvas drawing tool state
  const [showDrawingTool, setShowDrawingTool] = useState(false);
  const [drawingQuestionNumber, setDrawingQuestionNumber] = useState<
    number | null
  >(null);
  ...
  const [imageSelectionQuestionNumber, setImageSelectionQuestionNumber] =
    useState<number | null>(null);
  const { pendingCount, saveImageForReview, approvedImages } =
    useAIImageLibrary();
  ...
  const loadWorksheet = (worksheet: SavedWorksheet) => {
    ...
    setIncludeCoordinateGeometry(
      worksheet.settings.includeCoordinateGeometry ?? false,
    );
    setUseAIImages(
      worksheet.settings.useAIImages ??
        worksheet.settings.includeGeometry ??
        false,
    );
    ...
    try {
      if (worksheet.is_shared) {
        ...
        setSavedWorksheets((prev) =>
          prev.map((w) =>
            w.id === worksheet.id
              ? { ...w, is_shared: false, share_code: null }
              : w,
          ),
        );
        ...
        setSavedWorksheets((prev) =>
          prev.map((w) =>
            w.id === worksheet.id
              ? { ...w, is_shared: true, share_code: shareCode }
              : w,
          ),
        );
  ...
  const generateClipart = async () => {
    ...
    try {
      ...
      for (let i = 0; i < updatedQuestions.length; i++) {
        ...
        setClipartStatus(
          `Generating clipart ${i + 1} of ${updatedQuestions.length}...`,
        );
      ...
      toast({
        ...
        description:
          "Some clipart could not be generated. You can continue without them.",
        variant: "destructive",
      });
    } finally {
  ...
  const regenerateClipart = async (questionNumber: number) => {
    const question = compiledQuestions.find(
  ...
  const questionsNeedingImages = compiledQuestions.filter(
    (q) => q.imagePrompt && !q.imageUrl,
  );
  const hasQuestionsWithImagePrompts = compiledQuestions.some(
    (q) => q.imagePrompt,
  );
  ...
  const updateEditablePrompt = (questionNumber: number, newPrompt: string) => {
    setEditablePrompts((prev) =>
      prev.map((p) =>
        p.questionNumber === questionNumber ? { ...p, prompt: newPrompt } : p,
      ),
    );
  };
  ...
  const generateImagesWithEditedPrompts = async () => {
    if (editablePrompts.length === 0) {
      toast({
        ...
        description:
          "All questions already have images or no diagrams are needed.",
      });
      return;
    }
    ...
    const updatedQuestions = compiledQuestions.map((q) => {
      const editedPrompt = editablePrompts.find(
    ...
    setImageGenerationProgress(0);
    setImageGenerationStatus(
      `Generating images for ${editablePrompts.length} questions...`,
    );
    ...
    try {
      ...
      if (data.results) {
        ...
        for (const result of data.results) {
          ...
          // Update progress
          const progress =
            ((data.results.indexOf(result) + 1) / data.results.length) * 100;
          setImageGenerationProgress(progress);
          setImageGenerationStatus(
            `Generated ${successCount} of ${editablePrompts.length} diagrams`,
          );
        }
  ...
  const [regeneratingQuestionTextNumber, setRegeneratingQuestionTextNumber] =
    useState<number | null>(null);

  // State for tracking recently corrected symbols per question
  const [correctedSymbols, setCorrectedSymbols] = useState<
    Record<number, string[]>
  >({});

  // Helper to detect encoding issues in question text BEFORE regeneration
  const detectEncodingIssues = (
    text: string,
  ...
  const regenerateQuestionText = async (questionNumber: number) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke(
        "generate-worksheet-questions",
        {
          body: {
            ...
            bloomLevels: question.bloomLevel
              ? [question.bloomLevel]
              : ["apply"],
      ...
      if (data.questions && data.questions[0]) {
        ...
        const detected = detectCorrectedSymbols(
          oldQuestionText,
  ...
  const generatePDF = async () => {
    ...
    try {
      // Get formulas if formula sheet is included
      const formulas = includeFormulaSheet
        ? getFormulasForTopics(
      ...
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-xs mb-4">
            Select topics from the list to add them to your worksheet. Click the
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setShowSavedWorksheets(false)}
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
                    ...
                    <div className="flex items-center gap-2 pt-1 border-t">
                      ...
                      {worksheet.is_shared && worksheet.share_code && (
                        <Button
                          ...
                          className="text-xs h-7"
                          onClick={() =>
                            copyShareLink(worksheet.share_code!, worksheet.id)
                          }
                        >
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">
              {isCompiled
                ? `${compiledQuestions.length} questions`
                : `${selectedQuestions.length} topic(s)`}
            </Badge>
          </div>
        </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button
                    type="button"
                    variant={
                      worksheetMode === "diagnostic" ? "default" : "outline"
                    }
                    className={`h-auto py-3 flex flex-col items-center gap-1.5 ${
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-blue-700">
                          All students receive the same adaptive questions at a
                          consistent difficulty level. Fair for quizzes and
                          tests where everyone is assessed equally.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-purple-700">
                          Questions span levels A-F to identify each student's
                          understanding. Use responses to generate personalized
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  {bloomFilter.length === 0 && (
                    <p className="text-xs text-destructive">
                      Select at least one cognitive level
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setIncludeCoordinateGeometry(e.target.checked)
                    }
                    className="rounded border-input"
                  />
                  <Label
                    htmlFor="includeCoordinateGeometry"
                    className="text-sm cursor-pointer"
                  >
                  ...
                  >
                    <span className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground ml-5">
                        Generates images for shapes, graphs, and coordinate
                        geometry
                      </span>
                    </span>
                  </Label>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <span className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground ml-5">
                            Generates high-quality images using advanced AI
                            model
                          </span>
                        </span>
                      </Label>
                ...
                <div className="space-y-2 pt-2 border-t border-dashed">
                  ...
                  <Select
                    value={marginSize}
                    onValueChange={(v) =>
                      setMarginSize(v as "small" | "medium" | "large")
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="small">
                        Small (12mm) - More content space
                      </SelectItem>
                      <SelectItem value="medium">
                        Medium (20mm) - Balanced
                      </SelectItem>
                      <SelectItem value="large">
                        Large (25mm) - More white space
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  {includeScrapPaper && (
                    <div className="ml-5 space-y-2">
                      <Label className="text-xs text-muted-foreground">
                        Problems per scrap page
                      </Label>
                      <Select
                        value={scrapPaperLayout}
                        onValueChange={(v) =>
                          setScrapPaperLayout(
                            v as "single" | "split-2" | "split-4",
                          )
                        }
                      >
            ...
            <>
              ...
              <div className="p-3 bg-muted/50 rounded-lg border">
                ...
                <div className="flex flex-wrap gap-1">
                  {BLOOM_LEVELS.map((bloom) => {
                    const count = compiledQuestions.filter(
                      (q) => q.bloomLevel === bloom.level,
                    ).length;
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {hasQuestionsWithImagePrompts && (
                    <Badge
                      variant={imagesGenerated ? "default" : "secondary"}
                      className="text-xs"
                    >
                      {imagesGenerated
                ...
                <ScrollArea className="h-64 rounded-md border p-2">
                  {compiledQuestions.map((question) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {question.bloomLevel && (
                          <Badge
                            ...
                            title={
                              getBloomInfo(question.bloomLevel).description
                            }
                          >
                      ...
                      <div className="flex items-start gap-2">
                        <div className="flex-1 relative">
                          <p className="text-sm font-serif leading-relaxed">
                            {renderMathText(
                              fixEncodingCorruption(question.question),
                            )}
                          </p>
                          {regeneratingQuestionTextNumber ===
                            question.questionNumber && (
                        ...
                        <Button
                          ...
                          onClick={() =>
                            regenerateQuestionText(question.questionNumber)
                          }
                          disabled={
                            regeneratingQuestionTextNumber !== null ||
                            regeneratingQuestionNumber !== null
                          }
                      ...
                      {(() => {
                        const encodingCheck = detectEncodingIssues(
                          question.question,
                        );
                        if (
                          encodingCheck.hasIssues &&
                            ...
                            <div className="mt-1.5 flex items-center gap-1.5 p-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-amber-600 dark:text-amber-400 truncate">
                                  Click regenerate (↻) to fix:{" "}
                                  {encodingCheck.issues.slice(0, 2).join(", ")}
                                  {encodingCheck.issues.length > 2 &&
                                    ` +${encodingCheck.issues.length - 2} more`}
                                </p>
                              </div>
                            </div>
                          );
                      ...
                      {correctedSymbols[question.questionNumber] &&
                        correctedSymbols[question.questionNumber].length >
                          0 && (
                          <div className="mt-1.5 flex items-center gap-1.5 animate-in fade-in slide-in-from-top-1 duration-300">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {correctedSymbols[question.questionNumber].map(
                                (symbol, idx) => (
                                  <Badge
                                    key={idx}
                                    variant="outline"
                      ...
                      {question.imageUrl && (
                        <div className="mt-2 flex flex-col items-center gap-2">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              regenerateSingleImage(question.questionNumber)
                            }
                      ...
                      {question.clipartUrl && (
                        <div className="mt-2 flex flex-col items-start gap-2">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              regenerateClipart(question.questionNumber)
                            }
                            disabled={regeneratingQuestionNumber !== null}
                          >
                            <RefreshCw className="h-3 w-3 mr-1" />
                  ...
                  >
                    {isGeneratingImages ? (
                      ...
                      <>
                        ...
                        Review & Generate {questionsNeedingImages.length}{" "}
                        Diagram{questionsNeedingImages.length > 1 ? "s" : ""}
                      </>
                    )}
                  </Button>
                </div>
              ...
              <Button
                ...
                variant="outline"
                onClick={() => {
                  setDrawingQuestionNumber(
                    compiledQuestions[0]?.questionNumber || 1,
                  );
                  ...
                  <div className="flex items-center justify-between p-2 bg-primary/10 rounded-lg border border-primary/20">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-primary">
                        {compiledQuestions.filter((q) => q.clipartUrl).length}{" "}
                        clipart added
                      </span>
                    </div>
                    <Button
                ...
                <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-purple-700">
                        After students complete this worksheet, record their
                        results in the "Differentiated Worksheets" tool to
                        generate personalized follow-up worksheets.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow =
                  "0 8px 30px rgba(0,0,0,0.7), 0 0 0 3px #b91c1c";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.backgroundColor = "#dc2626";
                e.currentTarget.style.transform = "scale(1)";
                e.currentTarget.style.boxShadow =
                  "0 6px 24px rgba(0,0,0,0.6), 0 0 0 2px #dc2626";
              }}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-muted-foreground">
                  {compiledQuestions.length} question
                  {compiledQuestions.length !== 1 ? "s" : ""} • Press ESC to
                  close
                </span>
              </div>
              <div
                  ...
                  onClick={() => {
                    ...
                    try {
                      ...
                      toast({
                        ...
                        description:
                          "Your worksheet has been downloaded as a PDF.",
                      });
                    } catch (error) {
                  ...
                  onClick={() => {
                    ...
                    try {
                      ...
                      toast({
                        title: "Error",
                        description:
                          "Failed to print worksheet. Please try again.",
                        variant: "destructive",
                      });
                    }
                  }}
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      overflow: "visible",
                      fontFamily:
                        "'Segoe UI Symbol', 'Apple Symbols', 'Noto Sans Math', 'Times New Roman', Georgia, serif",
                    }}
                  >
                    ...
                    {showAnswerLines && (
                      ...
                      >
                        ...
                        <span style={{ flex: 1 }}>
                          Write all work in the <strong>Work Area</strong>{" "}
                          boxes. Put your final answer in the{" "}
                        ...
                        >
                          ...
                          <div
                            style={{
                              ...
                              marginRight: "0.5rem",
                              fontFamily:
                                "'Noto Sans Math', 'STIX Two Math', 'Cambria Math', Symbol, 'Apple Symbols', 'Segoe UI Symbol', Georgia, serif",
                              lineHeight: "1.5",
                              fontSize: "0.95rem",
                              color: "black",
                              wordWrap: "break-word",
                    ...
                    {includeAnswerKey &&
                      compiledQuestions.some((q) => q.answer) && (
                        <div
                          className="mt-12 pt-8 border-t-2 border-dashed border-gray-300"
                          style={{ pageBreakBefore: "always" }}
                        >
                          ...
                          <p className="text-center text-xs text-red-600 italic mb-6">
      ...
      <Dialog open={showPromptEditor} onOpenChange={setShowPromptEditor}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Review and edit the AI prompts for each diagram before generating.
              Better prompts lead to more accurate diagrams.
            </DialogDescription>
          </DialogHeader>
              ...
              {editablePrompts.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <Label
                        htmlFor={`prompt-${item.questionNumber}`}
          ...
          <DialogFooter className="pt-4 border-t">
            ...
            <Button
              onClick={generateImagesWithEditedPrompts}
              disabled={editablePrompts.length === 0}
            >
              <ImageIcon className="h-4 w-4 mr-2" />
              Generate {editablePrompts.length} Diagram
              {editablePrompts.length > 1 ? "s" : ""}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showClipartDialog} onOpenChange={setShowClipartDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Would you like to add AI-generated clipart images to make your
              worksheet more engaging? Each question will get a themed clipart
              icon based on its topic.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-3">
            <div className="flex items-start gap-3 p-3 bg-muted/50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Simple, fun line art images that match each question's topic
                  to help students engage with the material.
                </p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              This will generate {compiledQuestions.length} clipart image
              {compiledQuestions.length > 1 ? "s" : ""} and may take a moment.
            </p>
          </div>
          ...
          <DialogFooter className="gap-2">
            <Button
              variant="outline"
              onClick={() => setShowClipartDialog(false)}
            >
              Skip for Now
            </Button>
      ...
      <Dialog open={showPushDialog} onOpenChange={setShowPushDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Send this worksheet as an assignment to students on NYClogic
              Scholar Ai. Configure XP and coin rewards.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="p-3 bg-muted/50 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground">
                {compiledQuestions.length} questions •{" "}
                {selectedQuestions.map((q) => q.topicName).join(", ")}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Coins students can spend in the NYClogic Scholar Ai store
              </p>
            </div>
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  const standardCode =
                    selectedQuestions.length > 0
                      ? selectedQuestions[0].standard
                      : undefined;
                  const topicName = selectedQuestions
                    .map((q) => q.topicName)
                    .join(", ");
      ...
      <AIImageReviewDialog
        ...
        topicFilter={
          imageSelectionQuestionNumber !== null
            ? compiledQuestions.find(
                (q) => q.questionNumber === imageSelectionQuestionNumber,
              )?.topic
            : undefined
        }
      />
