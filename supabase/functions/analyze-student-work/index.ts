...
// ═══════════════════════════════════════════════════════════════════════════════
// AI MODEL CONFIGURATION — FIXED MODEL NAMES
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// AI GATEWAY CALL — deterministic with temperature control
// ═══════════════════════════════════════════════════════════════════════════════
...
async function callLovableAI(
  messages: any[], apiKey: string, functionName = 'analyze-student-work',
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (preExtractedOCR && typeof preExtractedOCR === 'string' && preExtractedOCR.length > 0) {
      ...
      // Use callLovableAI for unified retry, logging, and temperature=0
      const analysisText = await callLovableAI(
        [{ role: 'system', content: sysPrompt }, { role: 'user', content: userContent }],
        LOVABLE_API_KEY, 'analyze-student-work-ocr', supabase, effectiveTeacherId,
        'standard', analysisProvider,
        { temperature: 0, modelOverride: 'google/gemini-2.5-flash' }
      );
    ...
    const analysisText = await callLovableAI(
      ...
      LOVABLE_API_KEY, 'analyze-student-work', supabase, effectiveTeacherId,
      'standard', analysisProvider
    );
